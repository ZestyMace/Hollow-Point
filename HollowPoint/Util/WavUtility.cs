...
public class WavUtility
{
    ...
    public static AudioClip ToAudioClip(byte[] fileBytes, int offsetSamples = 0, string name = "wav")
    {
        ...
        UDebug.AssertFormat(audioFormat == 1 || audioFormat == 65534,
    ...
    private static float[] Convert8BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
    {
        ...
        UDebug.AssertFormat(wavSize > 0 && wavSize == dataSize,
    ...
    private static float[] Convert16BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
    {
        ...
        UDebug.AssertFormat(data.Length == convertedSize, "AudioClip .wav data is wrong size: {0} == {1}",
        ...
        string dirname = "recordings")
    {
        ...
        UDebug.AssertFormat(bytes.Length == fileSize, "Unexpected AudioClip to wav format byte count: {0} == {1}",
    ...
    private static int WriteFileHeader(ref MemoryStream stream, int fileSize)
    {
        ...
        // Validate header
        UDebug.AssertFormat(count == total, "Unexpected wav descriptor byte count: {0} == {1}", count, total);
    ...
    private static int WriteFileFormat(ref MemoryStream stream, int channels, int sampleRate, ushort bitDepth)
    {
        ...
        // Validate format
        UDebug.AssertFormat(count == total, "Unexpected wav fmt byte count: {0} == {1}", count, total);
    ...
    private static int WriteFileData(ref MemoryStream stream, AudioClip audioClip, ushort bitDepth)
    {
        ...
        // Validate header
        UDebug.AssertFormat(count == total, "Unexpected wav data id byte count: {0} == {1}", count, total);

        // Write bytes to stream
        count += WriteBytesToMemoryStream(ref stream, bytes, "DATA");

        // Validate audio data
        UDebug.AssertFormat(bytes.Length == subchunk2Size, "Unexpected AudioClip to wav subchunk2 size: {0} == {1}",
    ...
    private static byte[] ConvertAudioClipDataToInt16ByteArray(float[] data)
    {
        ...
        UDebug.AssertFormat(data.Length * x == bytes.Length,
    ...
    public static ushort BitDepth(AudioClip audioClip)
    {
        ...
        UDebug.AssertFormat(bitDepth == 8 || bitDepth == 16 || bitDepth == 32,
    ...
    private static string FormatCode(ushort code)
    {
        switch (code)
        {
            ...
            default:
                UDebug.LogWarning("Unknown wav code format:" + code);
                return "";
        }
    }

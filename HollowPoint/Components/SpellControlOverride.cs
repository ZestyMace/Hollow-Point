...
internal sealed class SpellControlOverride : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Log.Error("SPELLCONTROLOVERRIDE EXCEPTION {0}", e);
        }
    }
    ...
    public void State_CycloneSpin()
    {
        ...
        Log.Debug("Enemy Count {0}", Stats.instance.enemyList.Count);
        ...
        foreach (GameObject enemy in Stats.instance.enemyList)
        {
            ...
            hpbb.bulletDegreeDirection = (float)angle;//URandom.Range(0, HeroController.instance.cState.onGround ? 180 : 360);
            //hpbb.appliesDamageOvertime = true;
            hpbb.isDagger = true;
            hpbb.size = new Vector3(0.65f, 0.65f, 1);
            hpbb.bulletSprite = BulletBehaviour.BulletSpriteType.dagger;
    ...
    public static IEnumerator ScreamAbility_SulphurStorm(int charges)
    {
        ...
        for (int burst = 0; burst < burstAmount; burst++)
        {
            ...
            for (int shells = 0; shells < shellsPerBurst; shells++)
            {
                ...
                GameObject shell = HollowPointPrefabs.SpawnBulletAtCoordinate(waveRotation + URandom.Range(-10f, 10f), knightPos, 0);
    ...
    public IEnumerator ScreamAbility_SparkingBullets(int charges)
    {
        ...
        while (sparkAmount > 0)
        {
            ...
            hpbb.bulletDegreeDirection = (float)fireBulletAtAngle + URandom.Range(-3, 3);
    ...
    public static IEnumerator ScreamAbility_CreepingAirburst(int charges)
    {
        ...
        for (int shells = 0; shells < totalShells; shells++)
        {
            ...
            hpbb.ignoreAllCollisions = true;
            hpbb.targetDestination = targetCoordinates + new Vector3(0, URandom.Range(5f, 6f), -0.1f);
    ...
    private IEnumerator QuakeAbility_SporeRelease(int charges)
    {
        ...
        for (int pulse = 0; pulse < pulseAmount; pulse++)
        {
            ...
            Mirror.SetField(dungCloud.GetComponent<DamageEffectTicker>(), "damageInterval", cloudIntensity);
    ...
    public IEnumerator QuakeAbility_BulletSpray(int charges)
    {
        ...
        {
            float degreeDeviation = URandom.Range(0, 180);
            GameObject bullet = HollowPointPrefabs.SpawnBulletFromKnight(URandom.Range(0, 180), DirectionalOrientation.Center);
            BulletBehaviour hpbb = bullet.GetComponent<BulletBehaviour>();
            hpbb.gunUsed = Stats.instance.currentEquippedGun;
            hpbb.bulletDamage = 2;
    ...
    public static IEnumerator QuakeAbility_DangerClose(int charges)
    {
        ...
        for (int shells = 0; shells < totalShells; shells++)
        {
            //GameObject shell = Instantiate(HollowPointPrefabs.bulletPrefab, targetCoordinates + new Vector3(URandom.Range(-5, 5), URandom.Range(25, 50), -0.1f), new Quaternion(0, 0, 0, 0));
            GameObject shell = HollowPointPrefabs.SpawnBulletAtCoordinate(270, knightPos + new Vector3(URandom.Range(-6, 6), 80, -1f), 0);
            BulletBehaviour hpbb = shell.GetComponent<BulletBehaviour>();
            hpbb.fuseTimerXAxis = true;
            hpbb.targetDestination = knightPos + new Vector3(0, URandom.Range(2f, 7f), -0.1f);
            hpbb.ignoreAllCollisions = true;

...
internal sealed class HollowPointPrefabs : MonoBehaviour
{
    public static GameObject bulletPrefab = null!;
    ...
    public static Dictionary<string, Sprite> projectileSprites = [];
    ...
    private IEnumerator GetFSMPrefabsAndParticles()
    {
        ...
        try
        {
            ...
            DontDestroyOnLoad(fireballImpactClone);
            ...
            DontDestroyOnLoad(spellParticlesClone);
            ...
            DontDestroyOnLoad(grimmParticleClone);
            ...
            DontDestroyOnLoad(furyParticles);
            ...
            DontDestroyOnLoad(furyBurst);
            furyBurst.SetActive(false);
            prefabDictionary.Add("FuryBurstPrefab", furyBurst);
        }
        catch (Exception e)
        {
            Log.Error("Getting the FSM prefabs was fucked my dude {0}", e);
        }
    ...
    public static GameObject SpawnObjectFromDictionary(string key, Vector3 spawnPosition, Quaternion rotation)
    {
        try
        {
            if (!GetPrefab(key, out var spawnedGO))
                return null!;
    ...
    private static bool GetPrefab(string key, out GameObject? go)
    {
        if (!prefabDictionary.TryGetValue(key, out go))
        {
            Log.Error("Could not find GameObject with key {0}", arg0: key);
            if (Log.IsEnabled(Modding.LogLevel.Fine))
            {
                Log.Trace("Only available keys are:");
                foreach (string keys in prefabDictionary.Keys)
                    Log.Trace(keys);
            }
            return false;
        }
